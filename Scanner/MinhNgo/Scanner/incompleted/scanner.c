...
Token* readIdentKeyword(void) {
  ...
  while (currentChar != EOF && (charCodes[currentChar] == CHAR_LETTER || charCodes[currentChar] == CHAR_DIGIT)) {
    if (cnt < MAX_IDENT_LEN) token -> string[cnt++] = (char) currentChar;
    readChar();
  }
...
Token* readNumber(void) {
  ...
  while (currentChar != EOF && charCodes[currentChar] == CHAR_DIGIT) {
    token -> string[cnt++] = (char) currentChar;
    readChar();
  }
  token -> string[cnt] = '\0';
  if (strlen(token -> string) >= strlen(MAX_INTEGER)) {
    if (strlen(token -> string) > strlen(MAX_INTEGER)) {
      error(ERR_INTEGEROVERFLOW, token -> lineNo, token -> colNo);
      return token;
    }
    if (strcmp(token -> string, MAX_INTEGER) > 0) {
      error(ERR_INTEGEROVERFLOW, token -> lineNo, token -> colNo);
      return token;
    }
  }
...
Token* readConstString() {
  ...
  while (charCodes[readChar()] != CHAR_DOUBLEQUOTE) {
    token -> string[cnt++] = currentChar;
    if (cnt == MAX_IDENT_LEN || currentChar == '\n') {
      error(ERR_INVALIDSTRINGCONSTANT, lineNo, colNo);
    }
  }
  token -> string[cnt] = '\0';
  readChar();
...
int tokenEq(char* token, char* string) {
  while (*token != '\0' && *string != '\0') {
    if (*token != toupper(*string)) break;
    token++; string++;
  }
  return (*token == '\0') && (*string == '\0');
}
