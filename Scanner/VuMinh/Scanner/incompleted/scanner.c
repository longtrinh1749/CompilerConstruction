...
Token* readIdentKeyword(void) {
  int ln = lineNo;
  int cn = colNo;
  char keywordStr[MAX_STRING_LEN] = "";
  int len = 0; 
  Token* token;

  while ((charCodes[currentChar] == CHAR_DIGIT || charCodes[currentChar] == CHAR_LETTER) && len <= MAX_STRING_LEN)
  {
    char c = currentChar;
    strncat(keywordStr, &c, 1);
    len += 1;
    readChar();
  }
  ...
  {
    TokenType tk_type = checkKeyword(keywordStr);
    if (tk_type == TK_NONE)
      tk_type = TK_IDENT;
...
Token* readNumber(void) {
  int ln = lineNo;
  int cn = colNo;
  char numberStr[MAX_STRING_LEN] = "";

  while (charCodes[currentChar] == CHAR_DIGIT)
  {
    char c = currentChar;
    strncat(numberStr, &c, 1);
    readChar();
  }

  Token* token; 
  char* p;
  long num = strtol(numberStr, &p, 10);
...
Token* readConstChar(void) {
  ...
  readChar();
  // currentChar is constant character
...
Token* getToken(void) {
  ...
  case CHAR_LPAR:
    ...
    if (charCodes[currentChar] == CHAR_COMMA)
    {
